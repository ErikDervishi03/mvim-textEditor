...
WINDOW* WindowManager::get_window(const WindowName& name) {
...
const std::map<std::string, WINDOW*>& WindowManager::get_windows() const {
    return windows;
}

int WindowManager::create_window(const WindowName& name) {
    std::string key = to_string(name);
...
int WindowManager::kill_window(const WindowName& name) {
    return destroy_window(name);  // Alias for destroy_window
}   


void WindowManager::refresh_separators() {
...
// Helper methods
void WindowManager::resize_windows() {
    int num_windows = windows.size();
...
WINDOW* WindowManager::getWindowAt(int y, int x) {
...
int WindowManager::destroy_window(const WindowName& name) {
    std::string key = to_string(name);
    auto it = windows.find(key);
    if (it == windows.end()) {
        return EXIT_FAILURE;
...
std::string WindowManager::to_string(const WindowName& name) {
